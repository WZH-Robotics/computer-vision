...
struct KCOLOR32
{
    ...
    unsigned char& operator[](int index) {
        ...
        case 1: return g;
        case 2: return b;
        default: throw std::out_of_range("Index out of range for KCOLOR32");
        }
    }
};
...
public:
    ...
    // and write the contents of the matrix
    bool    Read(FILE *fp);
...
//    KBlobNList*  Boundaries(int nType, KRect* rcpScan=0) const;
    ...
    KImageGray      Opening(int size);
    KImageGray      Closing(int size);
...
public:
    ...
    void            SaveHisto(const KArray<KHSI>& arHSI, const std::string& filename);
    void            ColorHistoEqualization(KArray<KHSI>& arHSI) const;
    void            ReadandConvertoHSI(KArray<KHSI>& arHSI, const char* imagePath);
    void            ColorHistoMatching(KImageColor& souceImg, KImageColor& targetImg);
};
