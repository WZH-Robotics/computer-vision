...
//void KImageColor::RGBtoNRG(const int& j, const int& i, KNRG& oNRG) const
//{

//    double dR   = (double)(_ppA[i][j].r)/256.0;
//    double dG   = (double)(_ppA[i][j].g)/256.0;
//    double dB   = (double)(_ppA[i][j].b)/256.0;
...
KImageColor& KImageColor::FromHSV(KArray<KHSV>& arHSV)
{
	...
	for(int i=0, ii=arHSV.Row(); ii; i++,ii--)
		for(int j=0, jj=arHSV.Col(); jj; j++,jj--)
		{
			...
			idx = (int)h;
            f = h-idx;          //|H'mod2 - 1|
            p = v*(1.0-s);      //v-vs = V - C = m
            q = v*(1.0-s*f);    //v-vsf = V - C|H'mod2 - 1| = X + V - C = X + m
            t=v*(1.0-s*(1.0-f));//v-v*s(1.0-f)=v-v*s+v*s*f = m - (X + m) + V = V - X
...
KImageColor& KImageColor::RGB2Hue(KArray<KHSI>& arHSI)
{
    ...
    for(int i=0, ii=arHSI.Row(); ii; i++,ii--)
        for(int j=0, jj=arHSI.Col(); jj; j++,jj--)
        {
            Hue = 255*arHSI[i][j].h;    // 0 ~ 255
            Sat = 255*arHSI[i][j].s;    // 0 ~ 255
            Int = 255*arHSI[i][j].i;    // 0 ~ 255
...
KImageColor& KImageColor::FromNRG(KArray<KNRG>& arNRG)
{
    ...
    for(int i=0, ii=arNRG.Row(); ii; i++,ii--)
        ...
        {
            Nor = 255 * arNRG[i][j].n;              // 255*(dR+dG+dB) = R+G+B
            Red = arNRG[i][j].r;                    // dR/(dR+dG+dB)  = R/(R+G+B)
            Green = arNRG[i][j].g;                  // dG/(dR+dG+dB)  = G/(R+G+B)

            _ppA[i][j].r = Red * Nor;               // R
            _ppA[i][j].g = Green * Nor;             // G
            _ppA[i][j].b = (1-(Red+Green)) * Nor;   // B
