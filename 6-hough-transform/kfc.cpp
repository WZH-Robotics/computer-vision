...
//KImageGray KImageGray::Opening(int size)//erode and dilate
//{
//    KImageGray igOut = *this;

//    switch(size){
//    case 0:
//        igOut = this.BinaryErode().BinaryDilate();

//        break;

//    case 1:
//        igOut = this.BinaryErode2().BinaryDilate2();
...
//KImageGray KImageGray::Closing(int size)//erode and dilate
//{
//    KImageGray igOut(this->Row(),this->Col());
//    KImageGray igOut1(this->Row(),this->Col());
...
//            {
//                if(igOut1._ppA[i][j])
//                    if(igOut1._ppA[i-1][j-1]==0 || igOut1._ppA[i-1][j]==0 || igOut1._ppA[i-1][j+1]==0 || igOut1._ppA[i][j-1]==0
//                        || igOut1._ppA[i][j+1]==0 || igOut1._ppA[i+1][j-1]==0 || igOut1._ppA[i+1][j]==0 || igOut1._ppA[i+1][j+1]==0)
...
KImageGray KImageGray::Boundary(int Mask, int Ground)
{
    ...
    switch (Bdr) {
    case 0 :        //Mask = 4n, Ground = foreground
        for(int i=1,ii=Row()-2; ii; i++,ii--)
            for(int j=1,jj=Col()-2; jj; j++,jj--)
            {
                if(_ppA[i][j])
    ...
    case 1:         //Mask = 4n, Ground = background
        for(int i=1,ii=Row()-2; ii; i++,ii--)
            for(int j=1,jj=Col()-2; jj; j++,jj--)
            {
                if(_ppA[i][j] == 0)
    ...
    case 2:         //Mask = 8n, Ground = foreground
        for(int i=1,ii=Row()-2; ii; i++,ii--)
            for(int j=1,jj=Col()-2; jj; j++,jj--)
            {
                if(_ppA[i][j])
    ...
    case 3:         //Mask = 8n, Ground = background
        for(int i=1,ii=Row()-2; ii; i++,ii--)
            for(int j=1,jj=Col()-2; jj; j++,jj--)
            {
                if(_ppA[i][j] == 0)
...
void MergeBlobs(KBlobList& lBlobList, int blobIdx1, int blobIdx2) {
    // Merge Blob at blobIdx2 into Blob at blobIdx1
    KBlob* blob1 = lBlobList[blobIdx1];
    KBlob* blob2 = lBlobList[blobIdx2];

    // Merge points from blob2 into blob1
    blob1->insert(blob1->end(), blob2->begin(), blob2->end());

//    // Delete blob2 and remove it from the blob list
//    delete blob2;
//    lBlobList.erase(lBlobList.begin() + blobIdx2);
}

KCOLOR32 GetColorForLabel(int labelSize)
{
    KCOLOR32 color;
